              ...
              <div className="p-8 flex flex-col h-full">
                <div className="flex flex-wrap text-sm text-gray-500 gap-4 mb-4">
                  {featuredEvent.time && (
                    <div className="flex items-center gap-2">
                      <i className="fa fa-clock-o text-[#39c8e2]"></i>
                      {featuredEvent.time}
                    </div>
                  )}
                  {featuredEvent.location && (
                    <div className="flex items-center gap-2">
                      <i className="fa fa-map-marker text-[#39c8e2]"></i>
                      {featuredEvent.location}
                    </div>
                  )}
                  {featuredEvent.attendees && (
                    <div className="flex items-center gap-2">
                      <i className="fa fa-user text-[#39c8e2]"></i>
                      {featuredEvent.attendees}
                    </div>
                  )}
                </div>
                  ...
                  {["Days", "Hours", "Minutes", "Seconds"].map(
                    (label, index) => (
                      <div key={label} className="text-center w-[70px]">
                        <div className="text-[28px] font-bold text-[#39c8e2] leading-none mb-1">
                          45
                        </div>
                        <div className="text-[12px] text-[#777] uppercase">
                          {label}
                        </div>
                      </div>
                    )
                ...
                <div className="flex justify-between items-center mt-auto border-t pt-4">
                  ...
                  <Link
                    ...
                    className="register-btn bg-[#39c8e2] text-white px-6 py-3 rounded-full font-semibold text-sm hover:bg-[#2ba8c0] transition-transform transform hover:-translate-y-1 shadow-md"
                  >
                    Register Now
                  </Link>
                </div>
              ...
              >
                <div className="relative h-[150px] overflow-hidden">
                  ...
                  <div className="event-date-badge absolute top-4 left-4 bg-white w-[50px] h-[60px] rounded shadow-md text-center z-10">
                    <span className="block bg-[#39c8e2] text-white text-[10px] font-bold uppercase py-1 rounded-t">
                      {event.date.month}
                    </span>
                    <span className="block text-[20px] font-bold py-1">
                      {event.date.day}
                    </span>
                ...
                <div className="p-5 flex flex-col flex-grow">
                  ...
                  <Link
                    href={event.link || "#"}
                    className="event-link text-[#39c8e2] font-semibold text-sm mt-auto flex items-center hover:translate-x-1 transition-all"
                  >
